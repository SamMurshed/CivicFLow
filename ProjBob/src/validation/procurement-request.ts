...
export const CATEGORY_CHECKLIST: Record<string, Array<{ key: string; label: string; description: string }>> = {
  'Information Technology': [
    { key: 'scope_of_work', label: 'Scope of Work', description: 'Detailed description of the IT work required.' },
    { key: 'technical_specs', label: 'Technical Specifications', description: 'Hardware/software requirements and standards.' },
    { key: 'security_requirements', label: 'Security Requirements', description: 'Data classification and security controls needed.' },
    { key: 'budget_justification', label: 'Budget Justification', description: 'Cost breakdown and pricing rationale.' },
  ],
  'Professional Services': [
    { key: 'scope_of_work', label: 'Scope of Work', description: 'Description of the professional services required.' },
    { key: 'qualifications', label: 'Vendor Qualifications', description: 'Required certifications, licences, or credentials.' },
    { key: 'budget_justification', label: 'Budget Justification', description: 'Fee schedule and cost justification.' },
    { key: 'timeline', label: 'Project Timeline', description: 'Proposed milestones and delivery schedule.' },
  ],
  'Construction & Engineering': [
    { key: 'scope_of_work', label: 'Scope of Work', description: 'Detailed construction or engineering requirements.' },
    { key: 'site_plans', label: 'Site Plans / Drawings', description: 'Architectural or engineering drawings.' },
    { key: 'permits', label: 'Permits & Approvals', description: 'List of required permits and regulatory approvals.' },
    { key: 'budget_justification', label: 'Budget Justification', description: 'Cost estimate with material and labour breakdown.' },
    { key: 'insurance', label: 'Insurance Requirements', description: 'Required coverage types and minimums.' },
  ],
  'Healthcare & Medical Supplies': [
    { key: 'scope_of_work', label: 'Scope of Work', description: 'Description of medical supplies or services needed.' },
    { key: 'compliance', label: 'Regulatory Compliance', description: 'FDA clearances and applicable compliance standards.' },
    { key: 'budget_justification', label: 'Budget Justification', description: 'Unit pricing and total cost estimate.' },
  ],
  default: [
    { key: 'scope_of_work', label: 'Scope of Work', description: 'Clear description of what is being procured.' },
    { key: 'budget_justification', label: 'Budget Justification', description: 'Cost estimate and justification.' },
  ],
};
...
export const procurementRequestSchema = z.object({
  ...
  description: nonEmpty('Description').max(
    5000,
...
export const ALLOWED_TRANSITIONS: Record<RequestStatus, string[]> = {
  draft: [],
  submitted: ['approve', 'reject', 'request_correction', 'place_on_hold'],
  under_review: ['approve', 'reject', 'request_correction', 'place_on_hold'],
  awaiting_correction: [],
  correction_submitted: ['approve', 'reject', 'request_correction', 'place_on_hold'],
